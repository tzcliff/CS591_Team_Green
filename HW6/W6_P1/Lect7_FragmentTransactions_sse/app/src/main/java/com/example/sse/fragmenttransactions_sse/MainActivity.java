...
//        f1 = (Frag_One) findViewById(R.id.frag1);  //Q: Why won't this work for fragments?  Does the fragment even exist in R.java? A: No, because the fragments are hosted by the activity. There are no existing fragment object which can be found by id, instead, we need to instantiate it.
...
//        f1 = (Frag_One) fm.findFragmentByTag("tag1");   //what should we do if f1 doesn't exist anymore?  How do we check and how do we fix? A: If it no longer exists, create a new instance of it. Check if it's null via an if statement. A: If it no longer exists, add it through fragment transaction again.Check if it's null via an if statement.
//
//    }
        // ft.replace(R.id.FragLayout, f1); we won't be using replace because it is more resource intensive

        if (fm.findFragmentByTag("tag2") != null) { // make sure f2 exists
            ft.detach(f2);
        }
...
//    ft.show(f1);   //why does this not *always* crash? A: Because f1 always exists and is shown during onCreate
    ...
    public void showFrag2() {
        ...
        ft.attach(f2);
        ft.addToBackStack("myFrag2");  //Q: What is the back stack and why do we do this? A: The back stack keeps track of previous Activities so that the back button works as the user expects it to A: The back stack keeps track of previous fragment transactions so that the back button works as the user expects it to
        ft.commit();
